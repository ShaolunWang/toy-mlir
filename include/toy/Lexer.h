...
public:
  ...
  /// Return the current identifier if (getCurToken() == tok_identifier)
  llvm::StringRef getId() {
    assert(curTok == tok_identifier);
    return identifierStr;
  }

  /// Return the current number (getCurToken() == tok_number)
  double getValue() {
...
private:
  ...
  /// track the current line number in the input stream
  int curLineNum = 0;

  /// track the current column number in the input stream
  int curCol = 0;
